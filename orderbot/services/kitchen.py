...
def get_dishes_count():
    ...
    for order in all_orders[1:]:
        # Проверяем, что заказ принят и на сегодня
        if order[2] == 'Принят' and order[11]:
            try:
...
def get_orders_summary():
    """
    Возвращает сводку по всем принятым заказам на текущий день, группируя блюда по приемам пищи.
    """
    # Получаем все заказы
    all_orders = orders_sheet.get_all_values()
    today = datetime.now().date()
