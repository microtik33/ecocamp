...
def get_dishes_count():
    ...
    for order in all_orders[1:]:
        # Проверяем, что заказ принят или ожидает оплаты и на сегодня
        if (order[2] == 'Принят' or order[2] == 'Ожидает оплаты') and order[11]:
            try:
...
def get_orders_summary():
    """
    Возвращает сводку по всем принятым заказам и заказам, ожидающим оплаты, на текущий день, группируя блюда по приемам пищи.
    """
    # Получаем все заказы
    all_orders = orders_sheet.get_all_values()
    today = datetime.now().date()
