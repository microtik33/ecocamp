...
MESSAGES = {
    ...
    'orders_display_error': 'Произошла ошибка при отображении списка заказов. Пожалуйста, попробуйте позже.',
    # Добавляем сообщения для оплаты через СБП
    'pay_with_sbp': '💳 Оплатите ваши заказы через СБП:',
    'pay_with_sbp_info': 'Для оплаты через Систему быстрых платежей отсканируйте QR-код с помощью банковского приложения или откройте QR-код в полном размере и сделайте скриншот для оплаты.',
    'sbp_payment_created': 'QR-код для оплаты создан! Отсканируйте его для оплаты {amount} руб.',
    'sbp_payment_success': '✅ Оплата на сумму {amount} руб. успешно получена! Спасибо за оплату заказов.',
    'sbp_payment_pending': '⏳ Ожидаем подтверждение оплаты...',
    'sbp_payment_error': '❌ Ошибка при создании QR-кода для оплаты: {error}',
    'sbp_payment_cancelled': '❌ Оплата отменена.',
    'sbp_payment_expired': '⌛ Время действия QR-кода истекло. Создайте новый QR-код для оплаты.',
    'sbp_api_not_configured': '⚠️ Оплата через СБП временно недоступна. Пожалуйста, обратитесь к администратору.'
}
...
BUTTONS = {
    ...
    # Добавляем кнопки для оплаты через СБП
    'pay_sbp': '💳 Оплатить через СБП',
